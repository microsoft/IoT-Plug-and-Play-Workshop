param([string] [Parameter(Mandatory=$true)] $mapSubscriptionKey,
      ...
      [string] [Parameter(Mandatory=$true)] $uniqueId,
      [string] [Parameter(Mandatory=$true)] $trackerUrl
)
...
$tracker = $trackerUrl + "?resGroup=$($resourceGroupName)&uniqueId=$($uniqueId)&progressMarker=5"
Invoke-WebRequest -Uri "$tracker" -Method Get
...
$tracker = $trackerUrl + "?resGroup=$($resourceGroupName)&uniqueId=$($uniqueId)&progressMarker=6"
Invoke-WebRequest -Uri "$tracker" -Method Get
